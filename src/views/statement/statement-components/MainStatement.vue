...
export default {
  ...
  methods: {
    ...
    isScrolling() {
      ...
      const allowance = 60;
      this.isSticky =
        headerHeight + this.statementTextHeight + allowance <=
        window.pageYOffset;
    },
  ...
  watch: {
    ...
    isSticky(newData) {
      ...
      if (newData) {
        this.stickySeeMore =
          this.statementTextHeight > this.stickStatementHeightLimit
            ? true
            : null;
      }
    },
  ...
  computed: {
    ...
    isActive() {
      return (
        this.relation &&
        this.relation["id"] * 1 === this.selectedStatementId * 1
      );
    },
    isPublished() {
    ...
    statement() {
      return this.relation && this.relation["statement"]
        ? this.relation["statement"]
        : null;
    },
    statementId() {
      return this.statement &&
        typeof this.statement["id"] !== "undefined" &&
        this.statement["id"]
        ? this.statement["id"]
        : null;
    },
    ...
    relationOpinionType() {
      return this.relation === null ||
        typeof this.relation["user_opinion"] === "undefined" ||
        this.relation["user_opinion"] === null
        ? -1
        : this.relation["user_opinion"]["type"];
    },
    ctPoints() {
      return this.statement &&
        typeof this.statement["ct_points"] !== "undefined"
        ? this.statement["ct_points"]
        : "00";
    },
    titleIds() {
      return (
    ...
    relationTypeSymbol() {
      ...
      const relationTypeIndex = this.findArrayIndex(
        relationTypeId * 1,
        this.relationTypes,
        "id"
      );
    ...
    userFollowingNames() {
      ...
      for (let userId in this.userFollowing) {
        ...
        if (typeof this.userFollowing[userId]["username"] !== "undefined") {
          ...
          typeof this.userFollowing[userId]["user"] !== "undefined" &&
          this.userFollowing[userId]["user"]
        ) {
