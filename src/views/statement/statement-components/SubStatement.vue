<template>
  ...
  <div
    ...
    <div
      v-if="relationData"
      class="flex-fill pl-1 pt-1 border-right-radius bg-white"
    >
      <div
        v-if="isActive && relation['user']"
        class="text-sm ml-2 pr-3 d-flex justify-content-end"
      >
        ...
        <div
          v-if="isActive && isVirtualRelation"
        ...
        <div v-if="isActive" class="">
          <span v-if="publishedAt" class="pl-2 text-light">
            ...
            &#8226;
            <span>{{
              timeSince(publishedAt + " UTC", 2592000000, "m d, Y")
            }}</span>
          </span>
          <span v-else>
            <fa icon="briefcase" />
            &#8226;
            <span>{{
              timeSince(relation["created_at"] + " UTC", 2592000000, "m d, Y")
            }}</span>
          </span>
        </div>
      </div>
      ...
      >
        <div class="d-flex align-items-center py-1">
          <div>
            <CircleLabel
              v-if="isUpdating"
              class="mr-1 no-text-selection"
              title="Updating statement. Please wait..."
              v-tooltip="'Updating statement. Please wait...'"
            >
            ...
            >
              <small v-if="showImpact" class="text-nowrap"
                >{{ userOpinionImpact }}%</small
              >
              <small
                v-if="relation['statement'] && showScope"
                style="line-height: 1"
              >
                {{
                  relation["statement"]["scope_id"]
                    ? scopes[
                        findArrayIndex(
                          relation["statement"]["scope_id"],
                          scopes,
          ...
          <div class="flex-fill c-pointer align-items-stretch">
            <div>
              <div
                ...
                class="d-flex text-dark text-left m-0 p-0"
              >
                <div class="pl-2" style="line-height: 1.8">
                  <RelationTypeLabel
                    class="p-1.5"
                    :relation-type-id="relation['relation_type_id'] * 1"
                  />
          ...
          <div class="d-flex my-auto align-self-center">
            ...
            <div class="d-inline-flex">
              <div v-if="isActive" class="mr-1">
                ...
                <CircleIconButton
                  v-if="
                    enableDragging && !relation['published_at'] && !isUpdating
                  "
                  icon="arrows-alt"
                  button-class="move-icon btn-light bg-whitesmoke text-primary"
                />
              </div>
              <div v-else class="ml-2 mr-2 align-self-center text-nowrap">
                <span
                  v-if="
                    ...
                    mainRelationData['published_at']
                  "
                  @click="showTooltip"
                  v-tooltip="{ content: 'This statement is private' }"
                  class="no-text-selection"
                  title="Private"
                >
                  <fa icon="briefcase" />
                </span>
                ...
                <span
                  v-else-if="isAuthorNotUser"
                  v-tooltip="relation['user']['username']"
                ...
                <OpinionIcon
                  v-if="relationOpinionType && !showCTOpinion"
      ...
      <div class="mt-1">
        ...
        <draggable
          ...
          :list="
            !hasVirtualRelations
              ? relation['relations']
              : relation['virtual_relation']['relations']
          "
          class="dragArea"
          :class="
            ((isPositiveStatement && isDraggingStatement === 1) ||
              (!isPositiveStatement && isDraggingStatement === 2)) &&
            !isActive
              ? 'isDragging'
              : ''
          "
...
export default {
  ...
  methods: {
    ...
    listChanged(event) {
      ...
      if (
        ...
        this.relationData["relations"][addedIndex]["parent_relation_id"] =
          this.relationId;
        RelationAPI.update({
    ...
    focusToSubRelation() {
      if (
        this.relationData &&
        this.hashRelationId === this.relationData["id"]
      ) {
        // the last hash is a number
        const relationContainer = document.getElementById(
          "relation-container-" + this.hashRelationId
        );
        const windowContainer =
          this.windowContainers[
            relationContainer.getAttribute("is-support") === "true" ? 0 : 1
          ];
        setTimeout(() => {
          this.statementClicked();
          if (relationContainer) {
            windowContainer.scrollTop =
              relationContainer.offsetTop - windowContainer.offsetTop - 10;
          }
        }, 100);
      }
    },
  ...
  watch: {
    hashRelationId() {
      if (
        this.relationData &&
        typeof this.expandedRelationIds[this.relationData["id"]] !== "undefined"
      ) {
        ...
        setTimeout(() => {
          if (this.relation && typeof this.relation["id"] !== "undefined") {
            ...
            if (typeof relationData !== "undefined") {
              ...
              if (
                typeof this.expandedRelationIds[relationData["id"]] !==
                "undefined"
              ) {
                this.showStatement = true;
              }
  ...
  computed: {
    ...
    isLocked() {
      // if < 0, do not show, if 1 context lock active here, if 2 no context lock set
      if (
        typeof this.relation["user_relation_context_locks"] !== "undefined" &&
        this.relation["user_relation_context_locks"].length
      ) {
        for (
          let x = 0;
          x < this.relation["user_relation_context_locks"].length;
          x++
        ) {
          if (
            this.relation["user_relation_context_locks"][x][
              "root_relation_id"
            ] *
              1 ===
            this.mainRelationData["id"] * 1
          ) {
            return 1;
          }
        }
    ...
    statementId() {
      return typeof this.relation["statement"] !== "undefined"
        ? this.relation["statement"]["id"]
        : "ERROR: Statement text not found";
    },
    statementText() {
      if (this.relation["virtual_relation_id"]) {
        // if root virtual relation
        if (
          this.relation["virtual_relation"] &&
          this.relation["virtual_relation"]["statement"]
        ) {
          return this.relation["virtual_relation"]["statement"]["text"];
        } else {
          // if no virtual relation found
          return (
            "ERROR: Virtual Relation has no statement. #" +
            this.relation["virtual_relation_id"]
          );
        }
    ...
    showSubStatements() {
      return (
        (this.showStatement || this.subRelations.length === 0) &&
        !(
          this.isDraggingStatement &&
          this.selectedStatementId === this.relationId * 1
        ) &&
        ((this.isDraggingStatement && !this.hasVirtualRelations) ||
          !this.isDraggingStatement)
      );
    },
    hasVirtualRelations() {
      return (
        ...
        this.relation["virtual_relation_id"] &&
        this.relation["virtual_relation"] &&
        this.relation["virtual_relation"]["relations"]
      );
    },
    subRelations() {
      if (this.relation) {
    ...
    userOpinions() {
      return typeof this.relation["user_opinions"] === "object"
        ? this.relation["user_opinions"]
        : [];
    },
    hasToggleableChildren() {
      return (
        this.subRelations.length &&
        (!this.hasFilterApplied ||
          (this.hasFilterApplied &&
            typeof this.parentRelationIdsWithPassedFilterChildren[
              this.relationId
            ] !== "undefined"))
      );
    },
    ...
    isDifferentAuthor() {
      return (
        ...
        this.mainRelationData["user_id"] * 1 !== this.relation["user_id"] * 1
      );
    },
    isAuthorFilterPassed() {
      return (
        this.relation === null ||
        typeof this.relation["is_author_filter_passed"] === "undefined" ||
        this.relation["is_author_filter_passed"]
      );
    },
    ...
    isFilteredOut() {
      let failedTextFilter =
        this.statementTextFilter !== "" &&
        this.statementText
          .toLowerCase()
          .indexOf(this.statementTextFilter.toLowerCase()) === -1;
    ...
    relationOpinionScoreRelation() {
      ...
      if (this.relation["user_opinion"]) {
        userOpinionScoreRelation =
          this.relation["user_opinion"]["opinion_calculated_column"][
            "score_relation"
          ];
      }
    ...
    titleIds() {
      return (
        "(" +
        this.relation["parent_relation_id"] +
        ") [" +
        this.relationId +
        "]"
      );
