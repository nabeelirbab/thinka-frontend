    ...
    <div class="fixed-bottom">
      <div
        :class="
          isPositiveStatement
            ? 'positive-statement'
            : isPositiveStatement === false && !isMainStatement
            ? 'negative-statement'
            : 'text-white'
        "
      ...
      >
        <div class="">
          <select
            v-if="
              !(
                isMainStatement &&
                relation['parent_relation_id'] === null &&
                relation['virtual_relation_id'] === null
              )
            "
        ...
        <div class="pt-2">
          ...
          <textarea
            ...
            :disabled="isLoading || (statement['id'] && mode === 'create')"
        ...
        >
          ...
          <template v-for="context in contexts">
            <option :value="context['id']" class="text-dark">
              {{ context["description"] }}
            </option>
          </template>
        </select>
        <div class="text-right">
          ...
          <button
            @click="cancel"
            :disabled="isLoading"
            class="btn btn-outline-dark py-1 px-2 px-3 ml-2"
          >
            Cancel
          </button>
        </div>
...
export default {
  ...
  methods: {
    ...
    enterPressed(e) {
      if (!e.shiftKey && e.keyCode === 13 && this.statement.text !== "") {
        // 13 is enter
        this.save();
      } else {
        this.cancel();
    ...
    createStatement(param) {
      ...
      StatementAPI.create(param)
        ...
        .catch((errorResult) => {
          if (
            typeof errorResult.response !== "undefined" &&
            typeof errorResult.response.status !== "undefined"
          ) {
            if (errorResult.response.status === 422) {
              // unprocessible entity
              let responseError = errorResult.response.data.error;
              if (responseError["code"] * 1 === 4) {
                // statement already existed error
                this.existingStatementPrompt(
                  responseError["message"]["statement"]
                );
              }
            }
          } else {
    ...
    updateStatement(param) {
      ...
      StatementAPI.post("/update-relation", param)
        .then((result) => {
          if (result["data"]) {
            let newSubStatement = param;
            newSubStatement["id"] = result["data"]["id"];
    ...
    reset() {
      ...
      this.statement["id"] = null;
      this.statement["text"] = "";
      this.statement["relation"]["relation_type_id"] = this.isPositiveStatement
        ? "2"
        : "1";
      setTimeout(() => {
    ...
    determineRelevanceWindow(relationType) {
      ...
      if (relationType["default_impact"] * 1 === 0) {
        // if 0, relevance window is determined by isPositivieStatement
  ...
  computed: {
    relationTypes() {
      ...
      if (
        ...
        RelationTypeAPI.cachedData.value &&
        ...
        relationTypes =
          RelationTypeAPI.cachedData && RelationTypeAPI.cachedData.value
            ? RelationTypeAPI.cachedData.value["data"]
            : [];
      }
      return relationTypes;
    },
    contexts() {
      return ContextAPI.cachedData.value
        ? ContextAPI.cachedData.value["data"]
        : [];
    },
    canSave() {
      return (
        ...
        (this.statement["relation"]["relation_type_id"] * 1 !== 0 ||
          this.statement["relation"]["parent_relation_id"] * 1 === 0) &&
        !this.isLoading
      );
    },
  },
