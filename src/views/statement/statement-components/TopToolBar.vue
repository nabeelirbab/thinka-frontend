<template>
  <div class="image-background">
    <div class="d-flex container px-2 align-items-center">
      <div class="flex-fill">
        <button v-if="!showSearchText && searchText === ''" @click="showSearchText = true"
          class="btn btn-square border-none shadow-none text-white py-1 icon-size" title="Search this tree.">
          <fa icon="search" />
        </button>
        <div v-else class="input-group input-group-sm ">
          <div class="input-group-preppend rounded-l-oval">
            <span v-if="isLoading" class="input-group-text rounded-l-oval border-r-none pl-3 pr-0 bg-white border-0"
              style="font-size: 1.02em;">
              <fa icon="spinner" spin />
            </span>
            <button v-else-if="searchText !== ''" @click="clearSearchText"
              class="rounded-l-oval border-r-none btn bg-white text-danger pl-3 pr-0 pt-1" type="button"
              style="height:2.31em;" title="Clear search text.">
              <fa icon="trash" />
            </button>
            <span v-else class="input-group-text rounded-l-oval border-r-none pl-3 pr-0 bg-white border-none border-0"
              style="font-size: 1.02em;">
              <fa icon="search" />
            </span>
          </div>
          <input ref="searchText" v-model="searchText" @keydown.esc="blurSearch"
            @keypress.enter="searchText === '' ? showSearchText = false : null" @keyup="typing"
            @focusout="searchText === '' ? showSearchText = false : null"
            class="form-control rounded-r-oval border-0 shadow-none" placeholder="Enter search criteria">
        </div>
      </div>
      <button @click="toggleReadingMode" class="shadow-none btn-primary btn py-0 px-1 mx-2" title="Reading Mode.">
        <fa icon="glasses" />
      </button>
      <button @click="goBack" :disabled="backHistory.length <= 1"
        class="chevron-circle-button shadow-none btn-square btn py-1 px-0" title="Back.">
        <div>
          <fa icon="chevron-left" />
        </div>
      </button>
      <button @click="goForward" :disabled="!forwardHistory.length"
        class="chevron-circle-button shadow-none btn-square btn py-1 px-0" title="Forward.">
        <div>
          <fa icon="chevron-right" />
        </div>
      </button>
      <div>
        <button @click="bookmark" :class="rootBookmarkId ? 'text-primary-2' : ''" :disabled="isBookmarkLoading"
          class="btn icon-size py-1 text-white btn-square px-2 shadow-none" title="Bookmark">
      ...
      <div>
        <button v-if="mainRelation" @click="(user && relationUserId !== user['id']) ? nonAuthorPublish() : publish()"
          :class="mainRelation['published_at'] ? 'text-primary' : ''" :disabled="isPublishing"
          class="btn icon-size py-0 text-white btn-square px-0 shadow-none" v-bind:title="titlePublish">
